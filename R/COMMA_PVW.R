...
COMMA_PVW <- function(Mstar, # Observed mediator vector
  ...
  mstar_model_data <- data.frame(x = x_matrix, c = c_matrix, z = z_matrix,
                                 y = obs_outcome,
                                 mstar_01 = ifelse(obs_mediator == 1, 1, 0))
  ...
  if(interaction_indicator == FALSE){
    
    # Duplicate the dataset
    actual_dataset <- data.frame(x = x_vector, m = 0, c = c_matrix, 
                                 y_01 = ifelse(obs_outcome == 1, 1, 0),
                                 mstar_01 = mstar_model_data$mstar_01)
    
    duplicate_dataset <- data.frame(x = x_vector, m = 1, c = c_matrix,
                                    y_01 = ifelse(obs_outcome == 1, 1, 0),
                                    mstar_01 = mstar_model_data$mstar_01)
  ...
  } else if(interaction_indicator == TRUE){
